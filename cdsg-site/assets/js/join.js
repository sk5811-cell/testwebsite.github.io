...
(() => {
  ...
  function highlightTrackCard() {
    $$('[data-track]').forEach(c => {
      ...
      c.style.borderColor = on ? 'rgba(69,224,180,.5)' : '';
      c.style.background  = on ? 'rgba(69,224,180,.06)' : '';
    });
  }
  ...
  function wireForm(formId, opts) {
    ...
    const flag = el => {
      el.style.setProperty('border-color', '#FF6B6B');
      el.style.setProperty('box-shadow', '0 0 0 3px rgba(255,107,107,.14)');
